...
FREE_LIMIT = 3
CREDITS_AMOUNT = 20  # сколько кредитов даёт один пакет
...
def init_db():
    with get_db() as conn:
        ...
        ''')
        # Migration: add new columns to existing DB safely
        for col, default in [('credits', '0'), ('free_exports_used', '0')]:
            try:
                conn.execute(f'ALTER TABLE paid_users ADD COLUMN {col} INTEGER NOT NULL DEFAULT {default}')
            except Exception:
                pass
        conn.commit()
...
def set_user_status(email, active, plan=None):
    ...
    with get_db() as conn:
        conn.execute('''
            INSERT INTO paid_users (email, active, plan, since, credits, free_exports_used)
            VALUES (?, ?, ?, ?, 0, 0)
...
def add_credits(email, amount):
    """Add credits to a user's account (creates row if not exists)."""
    email = email.lower()
    with get_db() as conn:
        conn.execute('''
            INSERT INTO paid_users (email, active, plan, since, credits, free_exports_used)
            VALUES (?, 0, 'credits', ?, ?, 0)
            ON CONFLICT(email) DO UPDATE SET
                credits = paid_users.credits + excluded.credits
        ''', (email, datetime.now().isoformat(), amount))
        conn.commit()


def use_credit(email):
    """Decrement 1 credit. Returns new credits count, or -1 if no credits."""
    email = email.lower()
    with get_db() as conn:
        row = conn.execute(
            'SELECT credits FROM paid_users WHERE email = ?', (email,)
        ).fetchone()
        if not row or row[0] <= 0:
            return -1
        new_count = row[0] - 1
        conn.execute(
            'UPDATE paid_users SET credits = ? WHERE email = ?', (new_count, email)
        )
        conn.commit()
        return new_count


def use_free_export(email):
    """Increment free_exports_used counter. Returns new count."""
    email = email.lower()
    with get_db() as conn:
        conn.execute('''
            INSERT INTO paid_users (email, active, plan, since, credits, free_exports_used)
            VALUES (?, 0, NULL, ?, 0, 1)
            ON CONFLICT(email) DO UPDATE SET
                free_exports_used = paid_users.free_exports_used + 1
        ''', (email, datetime.now().isoformat()))
        conn.commit()
        row = conn.execute(
            'SELECT free_exports_used FROM paid_users WHERE email = ?', (email,)
        ).fetchone()
...
def get_user_status(email):
    ...
    with get_db() as conn:
        row = conn.execute(
            'SELECT active, plan, credits, free_exports_used FROM paid_users WHERE email = ?', (email,)
        ).fetchone()
    if not row:
        return {'active': False, 'plan': None, 'credits': 0, 'free_exports_used': 0}
    return {
        'active': bool(row[0]),
        'plan': row[1],
        'credits': row[2] or 0,
        'free_exports_used': row[3] or 0,
    }
...
def webhook():
    try:
        ...
        if event_type in ('payment.succeeded', 'subscription.active'):
            ...
            if email:
                if plan == 'credits':
                    # One-time credits pack — add 20 credits, don't set active
                    add_credits(email, CREDITS_AMOUNT)
                    print(f'✅ Credits added: {email} (+{CREDITS_AMOUNT})')
                else:
...
def check_status():
    ...
    if not email:
        return jsonify({'active': False, 'credits': 0, 'free_exports_used': 0})

    return jsonify(get_user_status(email))


# ============================================================
# USE CREDIT (called after successful export)
# ============================================================
...
def api_use_credit():
    ...
    remaining = use_credit(email)
    if remaining < 0:
        return jsonify({'ok': False, 'error': 'No credits'}), 403

    return jsonify({'ok': True, 'credits': remaining})


# ============================================================
# USE FREE EXPORT (called after successful export for free users)
# ============================================================
@app.route('/api/use-free', methods=['POST'])
def api_use_free():
    data = request.get_json(force=True)
    email = (data.get('email') or '').lower()
    if not email:
    ...
    used = use_free_export(email)
    return jsonify({'ok': True, 'free_exports_used': used})
